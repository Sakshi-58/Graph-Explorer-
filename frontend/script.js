...
9. Check diagonal for negative cycles`
        ...
        function displayComparisonResults(results) {
            results.sort((a, b) => {
                if (a.time === Infinity) return 1;
                if (b.time === Infinity) return -1;
                return a.time - b.time;
            });

            let tableHTML = `
                <thead>
                    <tr class="text-left text-xs font-semibold uppercase text-gray-700 border-b-2">
                        <th class="py-2 px-2">Algorithm</th>
                        <th class="py-2 px-2 text-right">Time (ms)</th>
                        <th class="py-2 px-2 text-right">Ops</th>
            ...
            const fastestTime = results.filter(r => r.time !== Infinity && !r.error)[0]?.time;

            results.forEach(res => {
                const timeText = res.time === Infinity ? 'N/A' : res.time.toFixed(3);
                let costText = res.cost === null ? '—' : (res.cost === 'N/A' ? 'N/A' : (typeof res.cost === 'number' ? res.cost.toFixed(2) : res.cost));
                let rowColor = 'text-gray-700';

                if (res.cycle) {
                    costText = `<span class="font-bold text-red-500">Cycle</span>`;
                    rowColor = 'bg-red-50 text-red-700';
                } else if (res.error) {
                    rowColor = 'bg-yellow-50 text-orange-700';
                } else if (res.time === fastestTime) {
                    rowColor = 'bg-green-100 text-green-800 font-bold';
                }

                tableHTML += `
                    <tr class="${rowColor}">
                        <td class="py-2 px-2 text-xs font-medium">${res.name}</td>
                        <td class="py-2 px-2 text-xs text-right font-mono">${timeText}</td>
                        <td class="py-2 px-2 text-xs text-right font-mono">${res.operations}</td>
                        <td class="py-2 px-2 text-xs text-right font-mono">${costText}</td>
                    </tr>
                `;
            });
